...
import com.example.architectcoders.feature.detail.DetailViewModel
import com.example.architectcoders.feature.home.HomeScreen
import com.example.architectcoders.feature.home.HomeViewModel
...
fun Navigation() {
    ...
    val repository = remember {
        HogwartsRepository(
            remoteWizardsDataSource = ServerWizardsDataSource(
                HogwartsClient.instance
            ),
            localWizardsDataSource = RoomWizardsDataSource(
                app.db.wizardDao()
            )
        )
    }
    ...
    NavHost(
        ...
        composable<Home>(
            ...
            HomeScreen(
                viewModel = viewModel {
                    HomeViewModel(
                        FetchWizardsByHouseUseCase(
                            repository
                        ),
                        FetchFavoriteWizardsUseCase(
                            repository
                        )
                    )
                },
